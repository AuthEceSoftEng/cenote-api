module.exports = (filter, results) => {
	if (filter.operator === "ne") {
		...
		results = filter.property_value ? results.filter((el) => Object.keys(el).includes(filter.property_name)) : results.filter((el) => !Object.keys(el).includes(filter.property_name));
		...
		filter.operands.forEach((operand) => {
			if (operand.operator === "eq") {
				...
				// eslint-disable-next-line max-len
				results = operand.property_value ? results.filter((el) => Object.keys(el).includes(operand.property_name)) : results.filter((el) => Object.keys(el).includes(operand.property_name));
